...
export async function participantsUpdate({ id, participants, action }) {
 ...
 /* conn.sendButtonDoc(id, wm, text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Aʟᴅɪ X Aɪsʏᴀʜ', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
  ...
  let welcom = 'https://telegra.ph/file/b60eadd13fa22c701a01b.jpg'

  let godbye = 'https://telegra.ph/file/098069ac9b79fc35b5fbb.jpg'
  conn.sendButtonImg(id, await(await fetch(action === 'add' ? wel : lea)).buffer(), 'Group Messege', text, action == 'add' ? 'Selamat Datang 😇' : 'Sampai Jumpa 👋', action === 'add' ? '.intro' : 'heheh 🗿', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
