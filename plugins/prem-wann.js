...
let tekk = `\`\`\`➩ ekosistemss@gmail.com PW : TESTS1383 ${args[0] ? args[0].capitalize() : false}\`\`\` `
...
const sections = [
   {
	title: '◤◢◣◥◤◢◣◥◤◢◣◥◤◢◣◥◤◢◣◥◤◢◣◥◤◢◣◥◤◢◣◥',
	rows: [
	{title: `${(ch == true ? false : freefire) == true ? p : f}` + "F • Free Fire", rowId: ".akun freefire"},
	{title: `${(ch == true ? false : mobilelegends) == true ? p:f}` + "M • Mobile Legends", rowId: ".akun mobilelegends"},
	]
    },
]
...
case 'mobilelegends':
         if ((ch == true ? false : mobilelegends) == true) { 
	     if (!isPrems) return conn.sendButton(m.chat, txtprem, botdate, [['ʙᴜʏ ᴘʀᴇᴍɪᴜᴍ', '.premium'],['ᴏᴡɴᴇʀ', '.owner nomor']], m)
		 }
...
case 'freefire':
         if ((ch == true ? false : freefire) == true) { 
	     if (!isPrems) return conn.sendButton(m.chat, txtprem, botdate, [['ʙᴜʏ ᴘʀᴇᴍɪᴜᴍ', '.premium'],['ᴏᴡɴᴇʀ', '.owner nomor']], m)
		 }
